...
write_env(){
  ...
  local telegram_token="" telegram_chat="" openai_key="" openai_enabled="N" openai_budget="5" mode engine_mode
  mode="${PROJECT01_WORKER_MODE:-$(default_mode)}"
  ask "Engine mode [placeholder/real] default placeholder: " engine_mode "placeholder"
  if [[ "$engine_mode" == "real" ]]; then
    echo "WARNING: real mode requires the existing V5 PROJECT01 engine files to be present and wired. If not wired, workers will fail loudly."
  fi
...
PROJECT01_ENGINE_MODE=$engine_mode
PROJECT01_OHLCV_PATH=$DATA_DIR/mnq_1m.csv
PROJECT01_BBO_PATH=$DATA_DIR/mnq_bbo_1m.csv
...
ENV
  if [[ "$DRY_RUN" != "1" && "${PROJECT01_ALLOW_DEV_TOKEN:-0}" != "1" && "$token" == "dev-token" ]]; then
    echo "Refusing unsafe PROJECT01_CLUSTER_TOKEN=dev-token outside dry-run/dev mode"
    exit 1
  fi
...
harden_network(){
  if [[ "$NODE_NAME" == "PI" ]]; then
    run "ufw allow in on tailscale0 to any port 8080 proto tcp || true"
    run "ufw deny 8080/tcp || true"
  fi
}
...
file_transfer_tests(){
  ...
  if [[ "$NODE_NAME" == "PI" ]]; then
    for host in "$MEGATRON_IP" "$OPTIMUS_IP" "$STARSCREAM_IP"; do
      [[ -z "$host" ]] && continue
      if ssh -o BatchMode=yes -o ConnectTimeout=3 "$user_name@$host" 'echo ssh-ok' >/dev/null 2>&1; then
        if run "'$PROJECT_ROOT/scripts/test_file_transfer.sh' '$user_name@$host'"; then
          echo "PASS PI->${host} transfer"
        else
          echo "WARN PI->${host} transfer failed"
        fi
      else
        echo "WARN PI->${host} transfer skipped: SSH not configured"
      fi
    done
  else
    if ssh -o BatchMode=yes -o ConnectTimeout=3 "$user_name@$PI_IP" 'echo ssh-ok' >/dev/null 2>&1; then
      if run "'$PROJECT_ROOT/scripts/test_file_transfer.sh' '$user_name@$PI_IP'"; then
        echo "PASS ${NODE_NAME}->PI transfer"
      else
        echo "WARN ${NODE_NAME}->PI transfer failed"
      fi
    else
      echo "WARN ${NODE_NAME}->PI transfer skipped: SSH not configured"
    fi
  fi
}
