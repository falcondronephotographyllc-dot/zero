...
use crate::engine::{real_v5_adapter, EngineAdapter, PlaceholderEngineAdapter, RealEngineAdapter};
...
pub fn run_worker(node: &str, mode: WorkerMode) -> Result<()> {
    ...
    let engine_mode = engine_mode_from_env()?;
    if matches!(engine_mode, EngineMode::Real) && !real_v5_adapter::is_real_v5_wired() {
        return Err(anyhow!(
            "PROJECT01_ENGINE_MODE=real but real V5 adapter is not wired. Set PROJECT01_ENGINE_MODE=placeholder for cluster plumbing tests, or wire the real engine."
        ));
    }
...
pub fn execute_job(job: &Job, mode: WorkerMode) -> Result<WorkerResult> {
    ...
    match engine_mode_from_env()? {
        EngineMode::Placeholder => PlaceholderEngineAdapter::execute(job),
        EngineMode::Real => RealEngineAdapter::execute(job),
    }
}
...
pub fn engine_mode_from_env() -> Result<EngineMode> {
    let mode = std::env::var("PROJECT01_ENGINE_MODE").unwrap_or_else(|_| "placeholder".to_string());
    match mode.as_str() {
        "placeholder" => Ok(EngineMode::Placeholder),
        "real" => Ok(EngineMode::Real),
        _ => Err(anyhow!(
            "invalid PROJECT01_ENGINE_MODE={mode}; expected placeholder|real"
        )),
    }
}
