...
MVP uses SQLite-backed queue through the PI API. Redis is installed for V1 live queue upgrade but is not the active job queue yet.
...
- `POST /api/portfolios/correlation`
- `POST /api/runs/{run_id}/generate-jobs`

Engine boundary:

- `PROJECT01_ENGINE_MODE=placeholder|real`
- `placeholder` is the default for cluster plumbing tests.
- `real` fails loudly until the real V5 adapter is wired.
...
## Cluster Plumbing Test

Use this exact flow for safe end-to-end plumbing:

1. Set `PROJECT01_ENGINE_MODE=placeholder`.
2. Start PI coordinator services.
3. Start one worker, preferably `OPTIMUS`.
4. Create a run with `POST /api/runs/create`.
5. Generate jobs with `POST /api/runs/{run_id}/generate-jobs`.
6. Verify worker claims and completes a placeholder job.
7. Verify a row exists in `strategy_results`.
8. Add `MEGATRON` and repeat claim/complete checks.
9. Add `STARSCREAM` in `light_worker` mode and verify only S1/S2 claims.

Placeholder mode validates distributed plumbing only. It does not run real strategy discovery or the V5 search engine.

## Real Engine Activation Checklist

1. Copy real V5 source files into the repo.
2. Wire `RealEngineAdapter` in [real_v5_adapter.rs](/home/cybertron/Documents/zero/src/engine/real_v5_adapter.rs).
3. Run `cargo test`.
4. Run `cargo build --release`.
5. Validate data checksums on all workers.
6. Set `PROJECT01_ENGINE_MODE=real`.
7. Start MEGATRON and OPTIMUS first.
8. Keep STARSCREAM `dev_only` until stable.
9. Generate S1 jobs first.
10. Then expand to S2/S3.
